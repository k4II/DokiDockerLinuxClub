...
二、在k4ii提供的文件系统基础上，添加了更多的文件支持，经过测试，基本上常用的命令（/bin和/usr/bin里面有些命令我没有添加so库）都能实现。你可以理解为我为这个容器装载了一个辣鸡镜像。

三、说明一下，提供的文件系统只有小小的6m左右，并非完全的文件系统，缺少很多命令和环境（java/python等），提供的文件系统仅支持部分命令！如果测试的时候提示没有这个命令的话，不妨将你自己的那个命令以及关联库和配置文件复制到这个文件系统的对应位置上，并赋予正确的权限！

四、其实比较菜，我搞了老半天愣是没整明白端口转发怎么操作。计算机网络知识欠缺的很啊~
...
说明：看到几位代码都没放在根目录，了解到文件放在根目录可能不安全，所以修改了程序。

二、将rootfs.tar.gz和main.c移动到你想要的任何目录，并解压rootfs.tar.gz

三、将main.c放到rootfs目录里面去。
